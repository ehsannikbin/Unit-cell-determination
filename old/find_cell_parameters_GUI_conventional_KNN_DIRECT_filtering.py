...
def fast_objective_loop(patterns, U, H, G, K, tol_len_rel, tol_cos_abs, use_hard_fallback):
    n_cand = U.shape[0]
    ...
    # max_k_len must be at least 2 for the fallback to work if K=1
    max_k_len = max(2, 2 * K) 
    ...
    for p_idx in range(n_patterns):
        ...
        # --- 1. Filter l1 with KNN & Tolerance ---
        ...
        for m in range(start1, end1):
            cand = sorted_indices[m]
            if abs(pred_lens_all[cand] - l1) / (l1 + eps) <= tol_len_rel:
                idx1[count1] = cand
                count1 += 1

        # --- 2. Filter l2 with KNN & Tolerance ---
        ...
        for m in range(start2, end2):
            cand = sorted_indices[m]
            if abs(pred_lens_all[cand] - l2) / (l2 + eps) <= tol_len_rel:
                idx2[count2] = cand
                count2 += 1

        # --- 3. FALLBACK MECHANISM (O(1) Array lookup) ---
        if count1 == 0 or count2 == 0:
            if use_hard_fallback:
                ...
                if count1 == 0:
                    ...
                    idx1[0] = sorted_indices[p1]
                    idx1[1] = sorted_indices[p2]
                    count1 = 2
                ...
                if count2 == 0:
                    ...
                    idx2[0] = sorted_indices[p1]
                    idx2[1] = sorted_indices[p2]
                    count2 = 2
...
@njit(fastmath=True, cache=True)
def get_best_assignments(patterns, U, H, G, K, tol_len_rel, tol_cos_abs, use_hard_fallback):
    ...
    for p_idx in range(n_patterns):
        ...
        if count1 == 0 or count2 == 0:
            if use_hard_fallback:
                assignments[p_idx, 0] = -1
                assignments[p_idx, 1] = -1
                continue
            else:
                if count1 == 0:
...
def objective_wrapper(free, patterns, U, candidates, system, K, tol_len_rel, tol_cos_abs, use_hard_fallback):
    ...
    # Pass it to the loop
    return fast_objective_loop(patterns, U, candidates, G, K, tol_len_rel, tol_cos_abs, use_hard_fallback)
...
class OptimizationWorker(QObject):
    ...
    def run(self):
        try:
            ...
            # 4. Arguments
            args = (patterns, U, candidates, s['system'], s['k_neighbors'], s['tol_len_rel'], s['tol_cos_abs'], USE_HARD_FALLBACK)
            ...
            best_assignments = get_best_assignments(patterns, U, candidates, G_best, s['k_neighbors'], s['tol_len_rel'], s['tol_cos_abs'], USE_HARD_FALLBACK)
    ...
    def calculate_scores(self, patterns, U, candidates, free_params, s):
        ...
        G = vec6_to_G(vec6)
        return _fast_score_calc(patterns, U, candidates, G, s['k_neighbors'], s['tol_len_rel'], s['tol_cos_abs'], USE_HARD_FALLBACK)

@njit(fastmath=True)
def _fast_score_calc(patterns, U, H, G, K, tol_len_rel, tol_cos_abs, use_hard_fallback):
    n_cand = U.shape[0]
    GU_rows = np.zeros((n_cand, 3))
    for i in range(n_cand):
        for j in range(3):
    ...
    for p in range(len(patterns)):
        ...
        for m in range(start1, end1):
            cand = sorted_indices[m]
            if abs(pred_lens[cand] - l1) / (l1 + eps) <= tol_len_rel:
                idx1[count1] = cand
                count1 += 1
        ...
        for m in range(start2, end2):
            cand = sorted_indices[m]
            if abs(pred_lens[cand] - l2) / (l2 + eps) <= tol_len_rel:
                idx2[count2] = cand
                count2 += 1
...
class CrystalApp(QMainWindow):
    def __init__(self):
        ...
        # --- Row 1: K & Tolerances ---
        self.sb_k = QSpinBox(); self.sb_k.setRange(1, 100); self.sb_k.setValue(K_NEIGHBORS)
        self.sb_tol_cos = QDoubleSpinBox(); self.sb_tol_cos.setSingleStep(0.01); self.sb_tol_cos.setValue(0.15)
        
        grid.addWidget(QLabel("K-Neighbors:"), 1, 0)
        grid.addWidget(self.sb_k, 1, 1)
        ...
        self.combo_strat = QComboBox(); self.combo_strat.addItems(['rand1bin', 'best1bin'])
        ...
        grid.addWidget(self.chk_local_bias, 6, 0, 1, 2) # Spans 2 columns
    ...
    def update_algo_inputs(self, algo_name):
        ...
        for w in [self.lbl_maxfun, self.sb_maxfun, self.lbl_eps, self.sb_eps, self.chk_local_bias]:
