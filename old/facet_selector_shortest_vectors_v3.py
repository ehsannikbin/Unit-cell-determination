        ...
        with h5py.File(fname, 'r') as fh:
            ...
            for i in target_frames:
                try:
                    ...
                    if len(peaks) < cfg.get('min_spots', 15):
...
def process_file_paths(file_paths, max_patterns, num_processors, min_spots_val, progress_callback=None):
...
class QtViewer(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.min_spots_spin.setValue(15)
        # Add it to the thresh_layout or instr_layout:
        thresh_layout.addRow("Min spots to process", self.min_spots_spin)
        ...
        thresh_layout.addRow(self.show_ac_circles_cb)
    ...
    def load_files_list_dialog(self):
        ...
        self._set_loaded_data(file_paths, folder, max_patterns_val, num_proc_val, min_spots_val, progress_callback=update_progress)
        progress.setValue(progress.maximum())
        progress.close()

    def _set_loaded_data(self, file_paths, folder, max_patterns, num_processors, min_spots_val, progress_callback=None):
        images, peaks, centers, facets, tuples = process_file_paths(file_paths, max_patterns, num_processors, min_spots_val, progress_callback=progress_callback)
        self.images = images
        self.peaks = peaks
        self.centers = centers
        self.facets = facets
    ...
    def on_save_current_images(self):
        ...
        # We use a large figure size + high DPI to ensure resolution > original
        # e.g. 10 inches @ 300 DPI = 3000 pixels wide (usually covers 2k images)
        ...
        if self.show_facets_cb.isChecked() and f['p0'] is not None:
            ...
            if s0 is not None:
                info_text = f"s1={s0:.4f} Ainv, s2={s1:.4f} Ainv, ang={ang:.2f} deg"
        
        # Place text OUTSIDE the image (as xlabel or title) to prevent overlap
        if info_text:
            ax_diff.set_xlabel(info_text, fontsize=14, color='black', labelpad=10)

        ax_diff.set_title(f"Pattern {idx}", fontsize=14)
        # We keep axis ticks off for cleanliness, or on if preferred. Off is usually better for presentation.
        ax_diff.set_xticks([])
        ax_diff.set_yticks([])

        diff_path_annotated = os.path.join(folder, f"pattern_{idx:04d}_diff_annotated.png")
        # bbox_inches='tight' ensures the outside text is saved
