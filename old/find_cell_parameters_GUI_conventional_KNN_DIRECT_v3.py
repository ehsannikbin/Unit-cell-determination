...
# Number of nearest neighbors to search on EACH side of the target length.
# K=16 means 32 total length candidates are tested per observed vector.
K_NEIGHBORS = 4
...
def fast_objective_loop(patterns, U, H, G, K):
    n_cand = U.shape[0]
    ...
    # --- NEW: SORT LENGTHS ONCE PER DE ITERATION ---
    ...
    # --- NEW: ZERO-ALLOCATION ARRAYS FOR KNN ---
    # K = K_NEIGHBORS
    max_k_len = 2 * K
    idx1 = np.zeros(max_k_len, dtype=np.int64)
    idx2 = np.zeros(max_k_len, dtype=np.int64)
    ...
    for p_idx in range(n_patterns):
        ...
        # --- 1. Binary Search for l1 ---
        insert_idx1 = np.searchsorted(sorted_lens, l1)
        start1 = max(0, insert_idx1 - K)
        end1 = min(n_cand, insert_idx1 + K)
        len_idx1 = end1 - start1
        for m in range(len_idx1):
            idx1[m] = sorted_indices[start1 + m]

        # --- 2. Binary Search for l2 ---
        insert_idx2 = np.searchsorted(sorted_lens, l2)
        start2 = max(0, insert_idx2 - K)
        end2 = min(n_cand, insert_idx2 + K)
        len_idx2 = end2 - start2
        for m in range(len_idx2):
            idx2[m] = sorted_indices[start2 + m]
        ...
        # --- EVALUATE ALL NEIGHBORS (No Tolerances) ---
        for ii in range(len_idx1):
            ...
            for jj in range(len_idx2):
                ...
                # NOTE: tol_cos_abs check has been completely removed.
...
@njit(fastmath=True, cache=True)
def get_best_assignments(patterns, U, H, G, K):
    """Runs the KNN logic ONE time to extract the absolute best hkl pairs for the current G matrix."""
    n_cand = U.shape[0]
    GU_rows = np.zeros((n_cand, 3))
    for i in range(n_cand):
        for j in range(3):
    ...
    for p_idx in range(n_patterns):
        ...
        insert_idx1 = np.searchsorted(sorted_lens, l1)
        start1 = max(0, insert_idx1 - K)
        len_idx1 = min(n_cand, insert_idx1 + K) - start1
        for m in range(len_idx1): idx1[m] = sorted_indices[start1 + m]

        insert_idx2 = np.searchsorted(sorted_lens, l2)
        start2 = max(0, insert_idx2 - K)
        len_idx2 = min(n_cand, insert_idx2 + K) - start2
        for m in range(len_idx2): idx2[m] = sorted_indices[start2 + m]
...
def objective_wrapper(free, patterns, U, candidates, system, K):
    ...
    # Pass it to the loop
    return fast_objective_loop(patterns, U, candidates, G, K)
...
class OptimizationWorker(QObject):
    ...
    def run(self):
        try:
            ...
            # 4. Arguments
            args = (patterns, U, candidates, s['system'], s['k_neighbors'])
            ...
            if s['algorithm'] == 'Differential Evolution':
                ...
                def direct_callback(xk):
                    ...
                    if direct_callback.iter % 200 == 0:
                        t_el = time.time() - self.start_time
                        self.log(f"[DIRECT iter {direct_callback.iter}] time={t_el:.1f}s")
                
                direct_callback.iter = 0

                # Run DIRECT with the new GUI parameters
                res_global = direct(
                    objective_wrapper, 
                    bounds, 
                    args=args, 
                    maxiter=s['de_maxiter'], 
                    maxfun=s['direct_maxfun'],      
                    eps=s['direct_eps'],            
            ...
            best_assignments = get_best_assignments(patterns, U, candidates, G_best, s['k_neighbors'])
    ...
    def calculate_scores(self, patterns, U, candidates, free_params, s):
        ...
        G = vec6_to_G(vec6)
        return _fast_score_calc(patterns, U, candidates, G, s['k_neighbors'])

@njit(fastmath=True)
def _fast_score_calc(patterns, U, H, G, K):
    n_cand = U.shape[0]
    GU_rows = np.zeros((n_cand, 3))
    for i in range(n_cand):
        for j in range(3):
    ...
    for p in range(len(patterns)):
        ...
        for ii in range(len_idx1):
            ...
            for jj in range(len_idx2):
                ...
                # NOTE: tol_cos_abs check removed.
...
class CrystalApp(QMainWindow):
    def __init__(self):
        ...
        self.algo_layout = QFormLayout()  # Changed to self.algo_layout
        
        self.sb_M = QSpinBox(); self.sb_M.setValue(6)
        
        self.sb_k = QSpinBox()
        self.sb_k.setRange(1, 100) # Give it a reasonable bounds
        self.sb_k.setValue(K_NEIGHBORS)      # Default value
        self.sb_k.setToolTip("Number of nearest neighbors to search on each side of the target length.")
        
        self.combo_algo = QComboBox()
        self.combo_algo.addItems(['Differential Evolution', 'Direct'])
        
        self.sb_iter = QSpinBox(); self.sb_iter.setRange(10, 999999); self.sb_iter.setValue(1000)
        ...
        # --- DIRECT Specific Widgets ---
        self.sb_maxfun = QSpinBox(); self.sb_maxfun.setRange(1000, 10000000); self.sb_maxfun.setSingleStep(100000); self.sb_maxfun.setValue(2000000)
        self.sb_eps = QDoubleSpinBox(); self.sb_eps.setRange(1e-6, 1.0); self.sb_eps.setDecimals(4); self.sb_eps.setSingleStep(0.01); self.sb_eps.setValue(0.1)
        self.chk_local_bias = QCheckBox("Locally Biased (DIRECT-L)")
        self.chk_local_bias.setChecked(True)
        self.chk_local_bias.setToolTip("Uncheck this for Triclinic/Monoclinic to force broader space exploration.")
        ...
        # Add to Layout
        self.algo_layout.addRow("Max hkl (M):", self.sb_M)
        self.algo_layout.addRow("K-Neighbors:", self.sb_k)
        self.algo_layout.addRow("Global Search:", self.combo_algo)
        self.algo_layout.addRow("Max Iter:", self.sb_iter)
        
        self.algo_layout.addRow("Pop Size:", self.sb_pop)
        self.algo_layout.addRow("Processors:", self.sb_workers)
        self.algo_layout.addRow("Strategy:", self.combo_strat)
        
        self.algo_layout.addRow("Max Evals (fun):", self.sb_maxfun)
        self.algo_layout.addRow("Epsilon (eps):", self.sb_eps)
        self.algo_layout.addRow("", self.chk_local_bias)
        
        self.algo_layout.addRow(self.chk_outlier, self.sb_keep)
        
        algo_group.setLayout(self.algo_layout)
    ...
    def update_algo_inputs(self, algo_name):
        ...
        # Toggle DE Widgets
        for w in [self.sb_pop, self.sb_workers, self.combo_strat]:
            w.setVisible(is_de)
            lbl = self.algo_layout.labelForField(w)
            if lbl: lbl.setVisible(is_de)
            
        # Toggle DIRECT Widgets
        for w in [self.sb_maxfun, self.sb_eps, self.chk_local_bias]:
