...
def fast_objective_loop(patterns, U, H, G):
    n_cand = U.shape[0]
...
@njit(fastmath=True, cache=True)
def get_best_assignments(patterns, U, H, G):
...
def objective_wrapper(free, patterns, U, candidates, system):
    ...
    # Pass it to the loop
    return fast_objective_loop(patterns, U, candidates, G)
...
class OptimizationWorker(QObject):
    ...
    def run(self):
        try:
            ...
            best_assignments = get_best_assignments(patterns, U, candidates, G_best)
    ...
    def calculate_scores(self, patterns, U, candidates, free_params, s):
        ...
        G = vec6_to_G(vec6)
        return _fast_score_calc(patterns, U, candidates, G)
...
class CrystalApp(QMainWindow):
    def __init__(self):
        ...
        # --- DE Specific Widgets ---
        self.sb_pop = QSpinBox(); self.sb_pop.setRange(10, 500); self.sb_pop.setValue(40)
